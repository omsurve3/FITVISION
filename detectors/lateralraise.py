...
class LateralRaiseDetector(BaseExercise):
    ...
    def process(self, landmarks) -> dict:
        ...
        avg_abduction = sum(angles) / len(angles)

        if avg_abduction < self.DOWN_THRESHOLD:
            self.stage = "down"

        if avg_abduction > self.UP_THRESHOLD and self.stage == "down":
            self.stage = "up"
            self.reps += 1
        ...
        if left_abduction is not None and right_abduction is not None:
            symmetry_status = "EVEN" if abs(left_abduction - right_abduction) <= self.SYMMETRY_TOLERANCE else "UNEVEN"
        else:
            symmetry_status = "N/A"

        return {
            "reps": self.reps,
            "abduction_angle": int(avg_abduction),
